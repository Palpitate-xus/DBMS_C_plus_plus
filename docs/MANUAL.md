...
> 路由说明：基础 `ALTER TABLE`、`CREATE TABLE` 分区、RLS 可见性扫描和简单单表视图 `INSTEAD OF` DML 路径由统一执行链处理；触发器函数运行时、OWNER/CLUSTER/REPLICA 等子命令仍有 legacy 或简化路径，并在生产状态文档中单独列明。RLS 已支持默认 `WITH CHECK`、`PUBLIC` 角色、基础 `PERMISSIVE/RESTRICTIVE` 组合以及基于 `pg_authid` 的 `SUPERUSER/BYPASSRLS` 绕过和 `FORCE ROW LEVEL SECURITY`；完整角色/owner/ACL 组合语义仍未完成。
