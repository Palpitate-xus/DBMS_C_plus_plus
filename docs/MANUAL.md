...
> 数据目录说明：当前版本只接受 v2、8 KiB heap page 和当前 schema 格式。heap 文件头、页布局、line pointer 和 checksum 必须通过严格校验；损坏或截断文件 fail-closed。旧数据目录不会自动迁移；升级前请导出 SQL 或删除并重建数据目录。WAL 的 LSN 0 是合法首位置，事务提交会先刷盘 COMMIT WAL，再刷盘 CLOG 可见性；WAL/CLOG/fsync 失败时提交失败、追加 ABORT WAL 并回滚。
