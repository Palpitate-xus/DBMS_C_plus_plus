...
RLS 当前执行路径已补齐 PostgreSQL 基础策略组合：策略默认为 `PERMISSIVE`，`AS RESTRICTIVE` 策略要求同时通过；显式 `TO PUBLIC` 与空角色列表均按 PUBLIC 处理；`ALL`/`UPDATE` 省略 `WITH CHECK` 时继承 `USING`；表 owner、`SUPERUSER`/`BYPASSRLS` 绕过普通 RLS，`FORCE ROW LEVEL SECURITY` 重新强制执行。表 owner 已写入正式 schema 与 `pg_class.relowner`，`ALTER TABLE ... OWNER TO` 会同步更新两者；查询、更新、删除和结构化 DML 来源仍共享关系感知扫描，复杂对象 owner/ACL 组合语义仍属于差距。
