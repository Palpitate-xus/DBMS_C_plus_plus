...
> 本 DBMS 当前状态（2026-08-11）: 生产化重构进行中，统一回归基线 PASS=127 FAIL=0（125 个 C++ 测试 + 协议 E2E + 窗口函数 E2E）；v2/8 KiB 存储格式已统一，旧数据不迁移；DDL 提交失败现在会向执行器传播并恢复已启用的物理快照，但完整 DDL 依赖事务语义仍在建设。
