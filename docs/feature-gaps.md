...
- **类别**: 优化器 / 子查询
- **现状**: 未关联的单列 `IN`/`NOT IN` 已由主 SQL 路径识别并下推为结构化 `SemiJoinOp`/anti 模式；未关联的单表 `EXISTS`/`NOT EXISTS`（含简单内层 `WHERE`）已下推为 `ExistenceFilterOp`，内层过滤消费 Volcano `FilterOp`；关联子查询、`ANY/ALL`、标量子查询、row comparison 和复杂布尔组合仍回退 legacy
...
- **实现路径**:
  ...
  4. 补齐 `ANY/ALL`、标量子查询、关联性、row comparison 和完整 planner 入口
- **预估工作量**: 1-2 周
