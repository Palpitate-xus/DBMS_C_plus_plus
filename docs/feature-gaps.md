...
- **实现路径**:
  ...
  4. 迁移复杂 INSERT SELECT、复合/部分/索引推断 conflict target、目标表列/子查询 `DO UPDATE`/`WHERE`、复杂 RETURNING、UPDATE/DELETE 复杂表达式与多表语义，统一 `ModifyTable` 风格执行。
