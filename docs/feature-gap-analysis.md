...
| 类别 | 已实现 |
|------|--------|
| DDL | CREATE/DROP DATABASE/TABLE/INDEX/VIEW/SCHEMA/SEQUENCE/TYPE/DOMAIN，ALTER TABLE ADD/DROP/RENAME COLUMN/RENAME TO/SET SCHEMA/ALTER COLUMN SET/DROP DEFAULT/SET/DROP NOT NULL/ADD/DROP CONSTRAINT/ENABLE/DISABLE TRIGGER，CREATE TABLE AS SELECT/PARTITION OF/PARTITION BY，ATTACH/DETACH PARTITION，SUBPARTITION，DEFAULT PARTITION，TRUNCATE CASCADE/RESTART IDENTITY，COMMENT ON，CREATE TEMPORARY/UNLOGGED TABLE |
...
| 索引 | B+ 树主键索引、B+ 树二级索引、复合索引、索引列排序 (ASC/DESC)、Hash 索引、GiST 空间索引、GIN 倒排索引、BRIN 块范围索引、SP-GiST 四叉树索引、全文索引、部分索引、表达式索引、覆盖索引、唯一索引、CREATE INDEX CONCURRENTLY |
...
**当前定位**：P0 全部完成！已实现 SQL-92 几乎全部基础功能 + SQL:1999 核心扩展（CTE、窗口函数、派生表、标量子查询）+ MVCC ReadView + B+ 树/Hash 索引 + 复合索引 + 覆盖索引 + 行级锁 + 意向锁(IS/IX) + 元数据锁(MDL) + 间隙锁/Next-Key 锁 + 查询优化器（JOIN 顺序优化、谓词下推、直方图统计）+ 表分区（Range/List/Hash）+ 数据库级权限 + 列级权限 + UTF-8 字符集 + BINARY/BLOB + SSL/TLS + 密码哈希(MD5/SHA256) + 审计日志，对标 SQLite 3.x 水平。

**下一阶段目标**：P0 全部完成，P1 全部完成！已实现 SQL-92 基础功能 + SQL:1999 核心扩展（CTE、窗口函数、派生表、标量子查询）+ MVCC ReadView + B+ 树/Hash 索引 + 复合索引 + 覆盖索引 + 行级锁 + 间隙锁/Next-Key 锁 + 查询优化器（JOIN 顺序优化、谓词下推、直方图统计）+ 表分区（Range/List/Hash）+ 数据库级权限 + UTF-8 字符集 + BINARY/BLOB + SSL/TLS + 密码哈希。接下来推进 P2 增强功能。
