...
| 2026-06-28 | Phase 4 Wave 4.30a CREATE TYPE shell + DROP TYPE enum 修复：`parseCreateType` 支持无 `AS` 子句的 `CREATE TYPE name`（shell type）；`DdlExecutor::executeCreateType` 将 shell 类型写入 `{db}/.shell_types`；`DdlExecutor::executeDropType` 原先只调用 `dropCompositeType`，导致删除 enum 类型失败——现改为先尝试 `dropCompositeType`，失败再尝试 `dropEnumType`，仍失败再移除 shell type 旁路。新增 `tests/create_type_shell_test.cpp`：enum 类型创建→使用→DROP 成功、shell 类型创建/DROP、composite 类型 DROP 仍正常。全量套件通过。range/base 形式仍待后续。 |
