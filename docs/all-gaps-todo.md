...
- **2026-08-09**：DML AST executor 扩展：普通 INSERT、常量单表 UPDATE、简单谓词单表 DELETE 及三类 DML 的列投影 RETURNING 已接入；修复列引用被误写成 NULL 的 fallback 边界，补充 INSERT/UPDATE/DELETE 协议回归和 UPDATE/DELETE 尾随垃圾 fail-closed parser 测试。高级 DML、多表语义和复杂表达式仍待迁移。
