...
2026-08-20 续：⑩ P2-2 Bloom 索引 — `src/access/BloomIndex`（128 bit/entry、k=7 FNV-1a double hashing + 精确 key→rid 侧表，零假阴探测、'BLM1' 原子落盘）；引擎 create/drop/get + `.bloomidx` 元数据 + 缓存逐出；INSERT/UPDATE/DELETE 全路径维护；`collectEqualityIndexCandidates`/`hasEqualityIndex` 扫描接入；`USING bloom` DDL + 名称注册表（`tests/bloom_index_test.cpp`，PASS=162）。
