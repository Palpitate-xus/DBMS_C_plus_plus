...
| 1.1.4 | `ALTER TABLE` | 已支持 ADD/DROP/RENAME COLUMN、ALTER COLUMN TYPE(整表改写+转换预校验)、**OWNER TO**（旁路元数据）、**SET LOGGED/UNLOGGED**（schema 标志位）、**SET STORAGE/RESET**、**CLUSTER ON/SET WITHOUT CLUSTER**、**REPLICA IDENTITY**、SET/DROP DEFAULT/NOT NULL、ADD CONSTRAINT(CHECK/UNIQUE/FK/PRIMARY KEY，PK 含既有数据校验+索引重建)、DROP CONSTRAINT、TRIGGER/RLS/SET SCHEMA/分区；仍缺 `IF EXISTS`、`ONLY`、`INHERIT`、真正 **SET TABLESPACE** 迁移、统计目标、真正延迟约束队列 | ⚠️ |
