...
| 日期 | 摘要 |
|------|------|
| 2026-08-09 | DDL 创建失败安全：`StorageEngine::createTable` 现在检查 schema/heap/分区/TOAST/主键/唯一索引及 `tlist.lst` 初始化结果，失败时清理已写入的 relation 文件、序列、缓存和清单项；新增损坏 heap 失败回归。索引构建、完整 DDL 回滚和跨对象依赖事务语义仍待后续。 |
