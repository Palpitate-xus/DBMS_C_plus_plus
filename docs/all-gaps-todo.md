...
| 6.11 | `UPDATE FROM` / `DELETE USING` | 单源表 `UPDATE FROM`/`DELETE USING` 已由 AST 执行器支持来源别名、限定列、连接谓词和 `RETURNING`，并复用存储层更新/删除约束；复杂 JOIN/子查询、重复匹配的完整语义、并发细节仍待迁移 | ⚠️ |
