...
| 日期 | 摘要 |
|------|------|
| 2026-08-08 | 子查询执行路径推进：未关联单列 `IN`/`NOT IN` 下推为 Volcano `SemiJoinOp`/anti 模式；未关联单表 `EXISTS`/`NOT EXISTS` 下推为 `ExistenceFilterOp`，内层过滤及 NULL/空集边界加入单测与协议 E2E；标量、关联子查询、ANY/ALL、row comparison 和复杂组合仍待迁移。 |
...
| 6.5 | Subquery | 未关联单列 IN/NOT IN 已下推 Volcano semi/anti plan 并覆盖 NULL 语义；未关联单表 EXISTS/NOT EXISTS 已进入 ExistenceFilterOp；标量、ANY/ALL、复杂关联子查询、row comparison、array ANY/ALL 和复杂组合仍不完整 | ⚠️ |
