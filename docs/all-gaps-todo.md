...
| 1.1.4 | `ALTER TABLE` | 已支持 ADD/DROP/RENAME COLUMN、ALTER COLUMN TYPE(整表改写+转换预校验)、SET/DROP DEFAULT/NOT NULL、ADD CONSTRAINT(CHECK/UNIQUE/FK/PRIMARY KEY，PK 含既有数据校验+索引重建)、DROP CONSTRAINT、TRIGGER/RLS/SET SCHEMA/分区；仍缺 `IF EXISTS`、`ONLY`、`INHERIT`、`OWNER`、`TABLESPACE`、`REPLICA IDENTITY`、统计目标、真正延迟约束队列 | ⚠️ |
