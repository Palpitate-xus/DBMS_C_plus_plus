...
2026-08-20 续：⑨ P1-9 自定义成本函数 — `QueryPlanner::CostModel`（seq/random_page_cost、cpu_tuple/index_tuple/operator_cost、enable_nestloop/hashjoin/mergejoin、自定义成本 hook 负值回退），`estimateJoinCost` 全权委托；GUC 会话级 SET 原地修补（粘性覆盖）、SET GLOBAL 持久化、SHOW/pg_settings 暴露（`tests/cost_model_test.cpp`，PASS=161）。
2026-08-20 续：⑩ P2-2 Bloom 索引 — `src/access/BloomIndex`（128 bit/entry、k=7 FNV-1a double hashing + 精确 key→rid 侧表，零假阴探测、'BLM1' 原子落盘）；引擎 create/drop/get + `.bloomidx` 元数据 + 缓存逐出；INSERT/UPDATE/DELETE 全路径维护；`collectEqualityIndexCandidates`/`hasEqualityIndex` 扫描接入；`USING bloom` DDL + 名称注册表（`tests/bloom_index_test.cpp`，PASS=162）。2026-08-20 续：⑪ P2-4 连接池 — `src/network/ConnectionPool`（session/transaction/statement 三模式 BackendContext 池、阻塞唤醒、prepared statement 跨租借存续）；`executeForProtocol` 短租路径 + accept 循环 `max_client_conn` 上限；GUC `pool_mode`/`pool_size`/`max_client_conn` + `SHOW POOLS`（`tests/connection_pool_test.cpp`，PASS=163）。
2026-08-20 续：⑫ P2-5 逻辑解码 — `LogicalDecoder`（pgoutput 帧式 + test_decoding 文本双插件）、`PublicationCatalog`（FOR TABLE/ALL TABLES + 发布过滤）、`LogicalChangeStore`（peek/acknowledge/限界）；三写路径全列行像入队、COMMIT 流向逻辑槽/ROLLBACK 丢弃；`CREATE/DROP PUBLICATION|REPLICATION SLOT` + 4 个 SHOW 面（`tests/logical_decoding_test.cpp`，PASS=164）。
2026-08-21 续：⑬ P2-8 TDE — `PageCrypto`（SHA-256-CTR 密流 + EtM 认证，零外部依赖）、边车信封 `<file>.tde`（nonce+MAC/页，全零=明文页就地迁移）、`BufferPool` 读写钩子 fail-closed、keyring 0600 + GUC `tde_keyring` 引擎前装载、WAL 崩溃恢复验证（`tests/tde_test.cpp`，PASS=165）。
2026-08-17 批量差距关闭（10 项，feature-gaps.md P1-10~13 / P2-9~13 / P3-7 全部翻转 ✅，详见各条目「本批实现」注记）：① PG 语法 `PREPARE name [(types)] AS … $1..$n` / `EXECUTE name(args)` / `DEALLOCATE`（`utils/prepared_stmts`）；② EXPLAIN ANALYZE 每节点 `(actual time/rows/loops)`（IOperator 插桩覆盖 21 算子）+ BUFFERS 按查询增量；③ 统计消费：MCV 精确等值/直方图范围插值/eqjoinsel join 选择率接入成本与 EXPLAIN；④ 并行 ParallelHashJoinOp / ParallelGroupAggregateOp / GatherMergeOp（ORDER BY 分区排序+归并）接入 planner；⑤ deferrable UNIQUE/FK（named+unnamed 约束形式、COMMIT 验证、SET CONSTRAINTS ALL IMMEDIATE）；⑥ `ARRAY[]` 构造/`a[lo:hi]` 切片/`@>`/`<@`；⑦ `CREATE TABLE … INHERITS` 列合并 + SELECT/UPDATE/DELETE `ONLY`；⑧ interval 算术（月+微秒双基）与 `AT TIME ZONE`；⑨ dollar-quoting `$tag$…$tag$`；⑩ 全文检索 `@@`/`to_tsvector`/`to_tsquery`/`plainto_tsquery`/`ts_rank`。回归基线 PASS=153 FAIL=0（新增 8 个 C++ 测试 + 1 个 E2E）。剩余次级缺口已在 feature-gaps.md 各条目标注（如 unnest、GIN 倒排、worker 池、多表 join DP、TimeZone GUC 等）。
...
2026-08-13 DDL 路由收敛：删除 `main.cpp` 中已被 typed bridge 完整覆盖的
`CREATE/DROP DOMAIN`、`CREATE/DROP SEQUENCE`、`CREATE/DROP SCHEMA`、数据库和角色/用户字符串
...
| 2026-08-12 | DDL 隐式提交错误传播补强：规范执行器及 `main.cpp` legacy DDL 分发统一检查隐式 `commitTransaction()` 失败状态；延迟约束/WAL/CLOG/fsync/SSI 失败时停止 DDL，新增 deferred CHECK + `CREATE DATABASE` 回归。 |
| 2026-08-12 | CLOG 提交失败传播补强：`CommitLog::flush()`/`setStatuses()` 返回持久化结果；COMMIT WAL 后 CLOG 刷盘失败时追加 ABORT WAL、执行 undo 并返回 `IO_ERROR`，恢复按最终 WAL 状态 fail-closed；新增删除 `pg_xact` 后提交与重启回归。 |
...
| 2026-08-09 | 顶层 DML statement atomicity 收敛：普通 `INSERT`/`UPDATE`/`DELETE`/`MERGE`/`REPLACE` 及写 CTE 由 `execute()` 建立内部事务，成功自动提交，错误/异常自动回滚；递归触发器、视图 action 与 CTE 复用同一边界，协议补齐 `WITH` 结果集，并新增回归验证批量 INSERT/CTE INSERT 后续行冲突不会留下前序行。 |
...
| 2026-08-09 | `DROP INDEX` typed 收口：parser 支持 PostgreSQL name-only、多名称、`IF EXISTS`、`CASCADE/RESTRICT`，DdlExecutor 同步删除物理索引、`pg_class` 和依赖；新增 `.idxnames` 持久化 SQL 名称到单列/Hash 物理键的映射，删除 `main.cpp` 重复的普通索引删除分支。`DROP INDEX CONCURRENTLY` 的两阶段事务/快照语义仍待后续。 |
| 2026-08-09 | 专用索引 typed 收口：标准 `CREATE INDEX ... USING HASH/GIN/GiST/BRIN/SPGiST` 统一进入 DdlExecutor，调用对应 StorageEngine 实现并登记 SQL 名称；删除 `main.cpp` 中非 PostgreSQL 的 `CREATE/DROP GIN/GiST/BRIN/SPGiST INDEX` 特殊分支。访问方法的泛化 opclass、WAL-safe 构建和并发维护仍待后续。 |
| 2026-08-09 | 索引分发再次收敛：删除 `main.cpp` 中约 138 行已被 DDL bridge 遮蔽的标准 `CREATE/UNIQUE INDEX` 字符串解析、事务提交和物理创建分支；标准索引语句现在只有 parser → DdlExecutor 一条执行链，`CREATE HASH INDEX` 等非 PostgreSQL 入口不再保留。 |
...
| 2026-08-07 | 扩展查询生命周期与结果元数据补强：Describe 严格校验 statement/portal 目标并返回参数描述，Close 真正删除 statement/portal 及其依赖资源；常见单表结果返回 catalog/table schema 驱动的 OID、长度、属性号和表 OID。复杂表达式的完整 RowDescription 类型推导和复杂二进制格式仍待后续。 |
...
| 1.1.22 | `CREATE POLICY` | 已迁移到 `DdlExecutor`：`parseCreatePolicy` 解析 `ON table / AS PERMISSIVE|RESTRICTIVE / FOR cmd / TO roles / USING / WITH CHECK`，`executeCreatePolicy` 校验表存在并写入 RLS policy 文件；USING/WITH CHECK 已通过关系感知扫描接入查询/更新/删除和结构化 DML 来源关系，默认 WITH CHECK、PUBLIC、基础策略组合、INHERIT/NOINHERIT、表 owner/角色属性绕过、无策略/求值失败 fail-closed 均已验证；仍缺少复杂 owner/ACL 组合和 ALTER POLICY 完整语义 | ⚠️ |
| 1.1.23 | `CREATE PROCEDURE` | 基本创建已落地，`DdlExecutor` 按分号切分 body 并调用 `createProcedure`；仍缺少语言运行时、事务控制规则、异常、变量、权限属性 | ⚠️ |
| 1.1.24 | `CREATE ROLE` / `CREATE USER` | 已写入 `pg_authid`，成员关系写入 `pg_auth_members`；主要角色属性、SCRAM 密码、`VALID UNTIL`、连接数限制、递归成员匹配和角色 ADMIN OPTION 基础授权已接入；仍缺完整 ACL 和 owner/依赖语义 | ⚠️ |
...
| 6.11 | `UPDATE FROM` / `DELETE USING` | 单源表及来源 INNER/CROSS JOIN 的 `UPDATE FROM`/`DELETE USING` 已由 AST 执行器支持来源别名、限定列、连接谓词和 `RETURNING`，并复用存储层更新/删除约束；外连接/复杂 JOIN/子查询、重复匹配的完整语义、并发细节仍待迁移 | ⚠️ |
...
| 8.2 | B-tree | 基础文件 B+Tree 已修复跨叶/内部节点分裂、重复键查找、范围扫描和多值 `(key, RID)` 精确删除；仍缺 dedup、suffix truncation、page deletion/合并、visibility map 驱动 index-only、skip scan 完整实现、NULLS FIRST/LAST 存储控制、collation/operator class | ⚠️ |
...
| # | 领域 | 差距描述 | 状态 |
|---|------|---------|------|
| 11.1 | 用户/角色 catalog | 已统一到 `pg_authid`/`pg_auth_members`；主要角色属性、SCRAM、password expiration、连接数限制、原始 membership、`INHERIT/NOINHERIT` 有效权限边界和角色 ADMIN OPTION 已执行；完整 ACL、owner/依赖语义仍缺 | ⚠️ |
