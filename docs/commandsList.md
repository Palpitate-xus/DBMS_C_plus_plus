...
**说明** 插入时若单列主键/唯一列冲突，则执行 UPDATE 操作。本项目当前结构化执行器支持普通 `VALUES`、单列主键/唯一列 conflict target、常量或直接 `excluded.column` `SET` 且无 `WHERE` 的窄版；带运算/函数的 `excluded` 表达式、列间表达式、复合或部分索引 target 等复杂语义仍走 legacy 路径。本质是 `INSERT ... ON CONFLICT` 语法，不是独立的 `UPSERT` 命令。
