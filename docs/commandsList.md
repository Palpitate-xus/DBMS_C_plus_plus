...
**说明** 插入时若单列主键/唯一列冲突，则执行 UPDATE 操作。本项目当前结构化执行器支持普通 `VALUES`、单列主键/唯一列 conflict target、常量或只引用 `excluded` 的 evaluator 受限标量表达式 `SET`，以及只引用目标行/`excluded` 的受限 `WHERE`；引用子查询或其他关系、复合或部分索引 target 等复杂语义仍走 legacy 路径。本质是 `INSERT ... ON CONFLICT` 语法，不是独立的 `UPSERT` 命令。
