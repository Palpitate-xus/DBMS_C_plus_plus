...
- DML AST 回归：`parser_phase1_test.cpp` 验证 `INSERT` 多行 AST、混合 `DEFAULT` 位置、`DEFAULT VALUES` 区分以及 UPDATE/DELETE 尾随垃圾 fail-closed；协议 E2E 验证普通单表 INSERT 的多行/表达式/DEFAULT、显式 NULL 不被默认值覆盖、字符串大小写保留、常量 UPDATE、简单谓词 DELETE、INSERT/UPDATE/DELETE 的列投影 `RETURNING`、多行 INSERT 返回结果和 `INSERT 0 N` command tag。
