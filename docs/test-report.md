...
- 窗口函数端到端测试：由统一回归入口执行 `tests/window_e2e_test.py`（10 用例，包含 Volcano `WindowAgg` 排名/偏移路径和 legacy frame/named-window 路径，使用隔离临时目录和临时管理员账号）
- Volcano 窗口单元测试：`tests/volcano_select_phase51_test.cpp` 覆盖多窗口独立排序、排名并列、分区边界和 `lag`。
