...
| UPDATE (FROM/LIMIT/RETURNING) | ✅ | ⚠️ | 常量表达式单表 UPDATE 走 `DmlExecutor` AST；列引用表达式、FROM/LIMIT/RETURNING、视图写入仍回退 legacy |
| DELETE (USING/LIMIT/RETURNING) | ✅ | ⚠️ | 简单谓词单表 DELETE 走 `DmlExecutor` AST；USING/LIMIT/RETURNING、ONLY、视图写入仍回退 legacy |
