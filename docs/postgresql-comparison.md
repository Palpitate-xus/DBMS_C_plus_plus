...
协议回归现已覆盖扩展查询错误后的 ignore-until-Sync 恢复、事务外 `ReadyForQuery('I')` 状态、文本整数参数绑定及 statement/portal 的 Describe/Close 生命周期；这只补齐了错误状态机与参数路径的一部分，二进制参数/结果、完整 RowDescription 类型映射、portal 分页和扩展消息仍与 PostgreSQL 有差距。
