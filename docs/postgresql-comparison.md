...
| TDE 透明数据加密 | ✅ | ✅ | 页级 SHA-256-CTR+EtM、边车信封、keyring 管理、崩溃恢复兼容（2026-08-21；索引旁路文件未覆盖、无密钥轮换） |
...
| **参数化路径** | ✅ | ❌ | 缺 |
| **自定义成本函数** | ✅ | ✅ | `QueryPlanner::CostModel`：5 个成本 GUC + enable 开关 + 自定义成本 hook（负值回退），join 选择全参数驱动（2026-08-20） |
| **遗传算法 join reorder** | ✅ | ❌ | 缺 |
| **分区裁剪** | ✅ | ✅ | ✅ |
