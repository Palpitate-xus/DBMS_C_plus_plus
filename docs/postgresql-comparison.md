...
| DELETE (USING/LIMIT/RETURNING) | ✅ | ⚠️ | 简单谓词单表 DELETE、单源表 DELETE USING 及列投影/受限标量表达式 RETURNING 走 `DmlExecutor` AST；复杂 USING、LIMIT、复杂/子查询/窗口 RETURNING、ONLY、视图写入仍回退 legacy |
...
- **DML Executor**: ⚠️ 普通 INSERT、受限行级标量表达式单表 UPDATE、单源表 UPDATE FROM、单源表 DELETE USING、简单谓词单表 DELETE 已进入 AST 执行器；高级 DML 和 MERGE 仍有 legacy 执行路径
