...
> 测试基线（2026-08-12）: PASS=135 FAIL=0（133 个 C++ 测试 + PostgreSQL 协议 E2E + 窗口函数 E2E；含 Volcano 算子、并发测试、数据库生命周期、schema 格式完整性、WAL 损坏恢复、WAL 归档失败重试、复制管理器并发状态和网络启动安全）
...
| 测试覆盖 | ~2000+ 测试 | 132 C++ 单元测试 + 2 个 E2E | 以回归结果为准 |
