...
| 功能 | PG 18 | 本 DBMS | 状态 |
|------|-------|---------|------|
| INSERT (VALUES/SELECT/ON CONFLICT/RETURNING) | ✅ | ⚠️ | 普通单表 VALUES/DEFAULT VALUES、无 JOIN/聚合/排序的 INSERT SELECT（含列表达式）、无 target 的 ON CONFLICT DO NOTHING 及列投影 RETURNING 走 `DmlExecutor` AST；复杂 SELECT、conflict target/DO UPDATE、RETURNING 表达式、视图写入和复杂未支持表达式明确回退 legacy |
