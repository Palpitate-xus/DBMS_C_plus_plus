...
- **Wave 4 仍进行中**：ALTER TABLE 全量子命令、CREATE TABLE 其余选项（access method、identity、PARTITION BY 执行）、CREATE TYPE（range/base/shell）、CREATE STATISTICS 的 ndistinct/mcv 算法与 catalog 落地、聚合/窗口函数完整集等。
