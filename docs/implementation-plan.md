...
| 🔄 4.27 补全 `ALTER TABLE` 全量子命令 | 1.1.4 | 已支持 ADD/DROP COLUMN（含 **`IF [NOT] EXISTS`** 守卫）、**ALTER COLUMN TYPE**（整表改写 + 转换预校验 + 索引重建）、**OWNER TO**（旁路元数据）、**SET LOGGED/UNLOGGED**（schema 标志位）、**SET/RESET STORAGE**、**CLUSTER ON/SET WITHOUT CLUSTER**、**REPLICA IDENTITY**、RENAME COLUMN/CONSTRAINT（含 **`IF EXISTS`**）、DROP CONSTRAINT（含 **`IF EXISTS`**）、ALTER COLUMN SET/DROP DEFAULT/NOT NULL、ADD CONSTRAINT（CHECK/UNIQUE/FK/**PRIMARY KEY**）、ENABLE/DISABLE TRIGGER、ROW LEVEL SECURITY、SET SCHEMA、ATTACH/DETACH PARTITION、**ALTER COLUMN SET STATISTICS**（存为 `column_statistics:col=n` 选项）；`ONLY`、`INHERIT`、真正的 **SET TABLESPACE** 迁移、真正延迟约束队列待补 |
...
| 🔄 5.2 实现等价类（equivalence classes）、pathkeys、参数化路径 | 7.2 | 基础 join 路径选择已就绪 |
| 🔄 5.3 实现 join search / join reordering（动态规划 / genetic） | 6.2, 7.2 | 基础 hash/merge/nested-loop join 已就绪 |
| 🔄 5.4 实现 bitmap heap scan、bitmap and/or、多索引组合 | 7.4 | IndexOnlyScan 已覆盖 covering index |
...
| 🔄 5.7 实现 `MERGE` 完整 WHEN 分支（BY SOURCE、DELETE、DO NOTHING、RETURNING） | 1.1.44, 6.13 | parser 已解析 MERGE；executor 未完整实现 |
...
| 🔄 5.36 实现 `LISTEN`/`NOTIFY`/`UNLISTEN` 事务提交后发送、payload、跨进程语义 | 1.1.42 | 内存 map 实现 |
...
| 🔄 7.8 实现 RLS executor-integrated 完整语义 | 11.6, 1.1.22 | DDL 注册已就绪 |
| 🔄 7.9 实现 SECURITY DEFINER/INVOKER、search_path 安全规则 | 11.7, 1.1.19 等 | — |
| 🔄 7.10 实现 `GRANT`/`REVOKE` ACL item 完整语义 | 1.1.40 | — |
| 🔄 7.11 实现 `ALTER USER`/`ALTER ROLE` 完整权限位（superuser/createdb/replication/bypassrls） | 1.1.5 | parser 已解析 |
