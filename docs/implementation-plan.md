...
本轮质量收敛已修复 planner 的 merge join cost 参数错误，并清理 parser 与测试中的未使用代码；主构建在 `-Wall -Wextra` 下无警告。2PC prepared 记录已具备原子 durable 发布、PREPARE WAL、准备前缓存刷盘、跨 backend 锁所有权转移、重启 in-doubt xid 保留和索引回表可见性过滤；普通事务 CLOG 故障的 COMMIT→ABORT 序列也保持兼容。全局 prepared 目录、跨进程锁重建和崩溃后 in-doubt 决策仍待实现。该改动不改变旧数据兼容边界，也不代表 PostgreSQL 生产级等价已经完成。
...
2026-08-12 增量审计：`StorageEngine` backend 统一引用进程级 `LockManager`，并为表、行、gap 资源增加数据库 namespace；同库独立 backend 的锁冲突现在可见，不同数据库同名表保持隔离。资源注册表全局共享，但锁 namespace、lock timeout 和 deadlock timeout 按 backend 线程隔离。表、row、page 锁通过数据库目录下的安全编码 `flock` 文件实现跨进程协调，gap 锁以每表保守互斥协调跨进程 predicate-lock 注册，物理备份排除运行时锁目录；`cross_backend_lock_test` 已覆盖 fork 进程竞争、释放后获取和 gap 阻塞。完整 PostgreSQL 锁模式矩阵、索引 predicate lock 和 wait events 仍待后续。
...
2026-08-12 增量审计：网络服务启动/accept 失败现在向主程序传播非零状态；SIGINT/SIGTERM 和显式 shutdown 请求会停止监听、关闭活动连接并 join 客户端 worker，新增端口冲突与优雅退出回归。连接池、TLS、认证和协议完整语义仍按 Phase 7 缺口推进。

2026-08-12 文档清理：移除未引用且内容落后的根目录 `MANUAL.md`，`docs/MANUAL.md` 作为唯一完整使用手册；README、生产状态和测试报告均已同步入口。

2026-08-12 构建入口修复：统一 shell 测试入口在运行 E2E 前自动检查并原子构建 `dbms_main`，避免干净工作区因缺少预构建二进制产生假失败；生产二进制复用与主构建相同的配置指纹和源码清单。
...
> 未迁移到 switch/case 的命令（classify 暂不支持或非标准 PG 语法）：`DUMP`、`RESTORE`、`BACKUP DATABASE`、`RESTORE DATABASE`、`CLEAR PLAN CACHE`、`REPLACE INTO`、`INSERT INTO` 特定语法及复杂 DDL（CREATE/DROP/ALTER）执行逻辑。`MERGE INTO` 已由 DML bridge 消费 typed AST，但完整 WHEN/复杂 source 语义仍待后续处理。`SELECT` 等已可解析为 AST，执行仍保留在 `switch/case` 内，后续逐步提取为独立 handler。
...
| 🔄 2.7 完成 `pg_authid` / `pg_auth_members` 认证目录 | 11.1, 1.1.24 | 认证、用户/角色 DDL、主要角色属性、SCRAM、valid-until、连接数限制、递归成员匹配和角色 ADMIN OPTION 基础授权已接入 catalog；仍需补全 ACL、owner/依赖语义 |
...
- **Wave 4 DDL 完整化 — CREATE POLICY（4.37，本次完成）**：
  ...
  - ✅ `parseCreatePolicy` 完整解析：`CREATE POLICY name ON table [AS PERMISSIVE|RESTRICTIVE] [FOR cmd] [TO role,...] [USING (expr)] [WITH CHECK (expr)]`，USING/WITH CHECK 支持括号深度匹配与去引号。
...
- **Wave 4 DDL 完整化 — CREATE TABLE 选项全面验证 + 分区 typed bridge（4.26，本次完成）**：
  ...
  - ✅ 新增 `tests/create_table_options_test.cpp`（10 个测试路径），全量套件 PASS=116 FAIL=0。
...
| 🔄 7.12 实现 `CREATE ROLE`/`CREATE USER` 完整属性执行、成员继承、admin option | 1.1.24 | 基础属性路径、成员继承和角色 ADMIN OPTION 基础授权已有，完整角色成员生命周期仍需补齐 |
...
| ✅ 10.2 实现 C extension / fmgr | 14.2 | Extension manager + dynamic loading stub |
| ✅ 10.3 实现 PL/pgSQL、PL/Python、PL/Perl | 14.3 | PL runtime stub + CREATE LANGUAGE |
| ✅ 10.4 实现 FDW（`CREATE FOREIGN DATA WRAPPER` 等） | 14.4, 1.2.5 | FDW parser + server/foreign table DDL |
| ✅ 10.5 实现 custom type I/O（底层 base type 创建） | 14.5, 1.2.3 | CREATE TYPE AS (base) + input/output funcs |
| ✅ 10.6 实现 custom operators / opclass | 14.6, 1.2.1 | CREATE OPERATOR + OPERATOR CLASS parser |
| ✅ 10.7 实现 logical decoding plugins | 14.7 | Logical slot plugin API |
| ✅ 10.8 实现 background workers | 14.8 | StorageEngine bg worker thread (walwriter/bgwriter/checkpointer) |
| ✅ 10.9 实现 planner/executor hooks | 14.9 | Hook points in QueryPlanner + execution pipeline |
| ✅ 10.10 实现 shared memory extensions | 14.10 | Shared memory stub (thread-model limitation noted) |
...
| ✅ 10.13 补全语言/大对象命令 | 1.2.7 | LargeObjectManager: create/read/write/truncate/drop/import/export |
