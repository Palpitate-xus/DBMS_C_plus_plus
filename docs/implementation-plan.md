...
> 当前审计（2026-08-09）：生产化重构进行中。已删除未接入的旧页式存储/迁移路径，统一使用 v2/8 KiB heap page；旧数据不兼容。文档中的历史 Wave 完成记录仅表示当时提交，不等于当前生产就绪。当前统一回归基线为 PASS=124 FAIL=0（122 个 C++ 测试 + 协议 E2E + 窗口函数 E2E）。普通单表 INSERT、常量 UPDATE、简单谓词 DELETE，以及普通单表 INSERT/UPDATE/DELETE 的列投影和受限标量表达式 RETURNING 已由 `DmlExecutor` 消费 AST，其余 DML 仍按明确回退边界逐步迁移。
...
| 🔄 1.5 补全 `INSERT/UPDATE/DELETE/MERGE` AST 路径 | 1.1.35, 1.1.41, 1.1.44, 1.1.58, 6.10~6.13 | AST 已完整建模；普通 INSERT、常量 UPDATE、简单谓词 DELETE，以及普通单表 INSERT/UPDATE/DELETE 的列投影和受限标量表达式 RETURNING 已由 `DmlExecutor` 消费，INSERT 高级子句、复杂/子查询/窗口 RETURNING、复杂 UPDATE/DELETE 和 MERGE 仍待迁移 |
...
- **Phase 2 已接入运行时（2026-06-22）**：
  ...
  - 明确延期：复杂 DML AST（复杂 INSERT SELECT、复合/部分/索引推断 conflict target、带运算/函数的 ON CONFLICT DO UPDATE/WHERE、复杂/子查询/窗口 RETURNING、多表 UPDATE/DELETE、MERGE）迁移、CTAS 语义补全、完整 FK `refobjid` 解析、`ALTER TABLE`/视图/触发器/函数/过程的目录集成仍为后续工作；普通 INSERT、简单单表 INSERT SELECT、无 target 的 ON CONFLICT DO NOTHING、单列主键/唯一列 target 的常量或直接 `excluded.column` DO UPDATE、常量/列表达式 UPDATE、简单 DELETE 及列投影/受限标量表达式 RETURNING 已进入结构化执行器。
...
| 🔄 5.8 实现 `INSERT` `DEFAULT VALUES`、`OVERRIDING`、conflict target/opclass/where | 1.1.41, 6.10 | 普通 VALUES/DEFAULT VALUES、简单单表 INSERT SELECT、无 target 的 ON CONFLICT DO NOTHING、单列主键/唯一列 target 的常量或直接 `excluded.column` DO UPDATE 及列投影/受限标量表达式 RETURNING 已新增 `DmlExecutor` AST 执行路径；OVERRIDING、复合/部分/索引推断 conflict target、带运算/函数的 DO UPDATE/WHERE、复杂 INSERT SELECT 和复杂/子查询/窗口 RETURNING 仍由 legacy 路径处理 |
