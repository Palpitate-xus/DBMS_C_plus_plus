...
| 🔄 4.20 实现聚合函数完整集（ordered-set、percentile、统计回归） | 3.7 | 新增 bool_and/bool_or/every、percentile_cont/percentile_disc（`tests/aggregate_bool_test.cpp` + `tests/aggregate_percentile_test.cpp`）；ordered-set（WITHIN GROUP 语法）、假设集聚合、统计回归聚合、并行聚合仍待后续 |
| 🔄 4.21 实现窗口函数完整语义（frame exclusion、RANGE/GROUPS、命名窗口） | 3.8 | 已支持 ROWS frame 的 `EXCLUDE CURRENT ROW`/`GROUP`/`TIES`/`NO OTHERS`、命名窗口 `WINDOW w AS (...)` 复用（含标准 SQL 子句顺序 `WHERE ... WINDOW ... ORDER BY`），覆盖 `tests/window_e2e_test.py`（9 用例全绿）；RANGE/GROUPS frame、ordered-set over 仍待后续 |
...
| 4.26 补全 `CREATE TABLE` 选项（`LIKE INCLUDING` 全集、`OF type`、access method、tablespace、identity、**PARTITION BY 执行测试**） | 1.1.28, 4.4 | 分区执行机制已实现并通过 `tests/partition_test.cpp` 覆盖；AST 桥接 `partitionBy` 仍待后续 |
| 🔄 4.27 补全 `ALTER TABLE` 全量子命令 | 1.1.4 | 已支持 ADD/DROP COLUMN（含 **`IF [NOT] EXISTS`** 守卫）、**ALTER COLUMN TYPE**（整表改写 + 转换预校验 + 索引重建）、**OWNER TO**（旁路元数据）、**SET LOGGED/UNLOGGED**（schema 标志位）、**SET/RESET STORAGE**、**CLUSTER ON/SET WITHOUT CLUSTER**、**REPLICA IDENTITY**、RENAME COLUMN/CONSTRAINT（含 **`IF EXISTS`**）、DROP CONSTRAINT（含 **`IF EXISTS`**）、ALTER COLUMN SET/DROP DEFAULT/NOT NULL、ADD CONSTRAINT（CHECK/UNIQUE/FK/**PRIMARY KEY**）、ENABLE/DISABLE TRIGGER、ROW LEVEL SECURITY、SET SCHEMA、ATTACH/DETACH PARTITION；`ONLY`、`INHERIT`、真正的 **SET TABLESPACE** 迁移、统计目标、真正延迟约束队列待补 |
...
| 4.30 补全 `CREATE TYPE`（enum/range/base/shell） | 1.1.32 | enum/composite 已可用，shell/range/base 元数据注册 + DROP TYPE 全类型修复已落地；作为列类型的完整运行时语义仍待后续 |
