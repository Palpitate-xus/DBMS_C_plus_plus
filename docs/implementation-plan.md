...
| 🔄 4.27 补全 `ALTER TABLE` 全量子命令 | 1.1.4 | 已支持 ADD/DROP COLUMN、**ALTER COLUMN TYPE**（整表改写 + 转换预校验 + 索引重建，`tests/alter_column_type_test.cpp`）、**OWNER TO**（旁路元数据）、**SET LOGGED/UNLOGGED**（schema 标志位）、**SET/RESET STORAGE**、**CLUSTER ON/SET WITHOUT CLUSTER**、**REPLICA IDENTITY**、ALTER COLUMN SET/DROP DEFAULT/NOT NULL、RENAME COLUMN/TABLE、ADD CONSTRAINT（CHECK/UNIQUE/FK/**PRIMARY KEY**，含既有数据去重/NULL 校验 + 物理 PK 索引重建，`tests/alter_add_pk_test.cpp`）、DROP CONSTRAINT、ENABLE/DISABLE TRIGGER、ROW LEVEL SECURITY、SET SCHEMA、ATTACH/DETACH PARTITION；`IF [NOT] EXISTS` 守卫、`ONLY`、`INHERIT`、真正的 **SET TABLESPACE** 迁移、统计目标、真正延迟约束队列待补 |
