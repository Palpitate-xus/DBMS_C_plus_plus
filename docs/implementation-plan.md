...
| ⚠️ 4.21 实现窗口函数完整语义（frame exclusion、RANGE/GROUPS、命名窗口） | 3.8 | 无 frame 的 `row_number`/`rank`/`dense_rank`/`lag`/`lead` 已由主入口接入 Volcano `WindowOp`；结构化计划支持 `WindowAgg` 文本/JSON EXPLAIN。窗口聚合、显式 frame/exclusion、RANGE/GROUPS、复杂目标列表、OFFSET 以及主 SQL EXPLAIN 的窗口解析仍待迁移，10 个窗口 E2E 覆盖两条执行路径。 |
