...
2026-08-12 增量审计：`StorageEngine` backend 统一引用进程级 `LockManager`，并为表、行、gap 资源增加数据库 namespace；同库独立 backend 的锁冲突现在可见，不同数据库同名表保持隔离。新增 `cross_backend_lock_test`，跨进程锁注册表和完整 PostgreSQL 锁模式矩阵仍待后续。
