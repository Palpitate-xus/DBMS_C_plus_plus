...
> 当前审计（2026-08-08）：生产化重构进行中。已删除未接入的旧页式存储/迁移路径，统一使用 v2/8 KiB heap page；旧数据不兼容。文档中的历史 Wave 完成记录仅表示当时提交，不等于当前生产就绪。当前统一回归基线为 PASS=122 FAIL=0（120 个 C++ 测试 + 协议 E2E + 窗口函数 E2E）。
...
| ⚠️ 5.12 实现 subquery 完整语义（关联子查询、row comparison、NULL 语义） | 6.5 | 未关联单列 IN/NOT IN 已进入 Volcano semi/anti plan 并覆盖 NULL 语义；未关联单表 EXISTS/NOT EXISTS 已进入 ExistenceFilterOp；单个未关联标量目标已进入 init-plan 并覆盖 NULL/cardinality error；复杂标量、ANY/ALL、关联子查询、row comparison 和复杂组合仍缺。 |
