...
| 🔄 4.27 补全 `ALTER TABLE` 全量子命令 | 1.1.4 | 已支持 ADD/DROP COLUMN、ALTER COLUMN SET/DROP DEFAULT/NOT NULL、RENAME COLUMN/TABLE、ADD CONSTRAINT（CHECK/UNIQUE/FK/**PRIMARY KEY** 新增，含既有数据去重/NULL 校验 + 物理 PK 索引重建，`tests/alter_add_pk_test.cpp`）、DROP CONSTRAINT、ENABLE/DISABLE TRIGGER、ROW LEVEL SECURITY、SET SCHEMA、ATTACH/DETACH PARTITION；ALTER COLUMN TYPE、IF [NOT] EXISTS 守卫、OWNER TO、SET TABLESPACE/LOGGED/STORAGE/STATISTICS、CLUSTER ON、REPLICA IDENTITY 待补 |
...
- **Wave 4 函数库 — format() 与空值合并（4.19i，本次完成）**：
  ...
  - 🔄 仍待后续：`format` 位置参数 `%n$s`、宽度/标志修饰符、`to_char`/`to_number` 数字/日期格式化。
...
- **Wave 4 仍进行中**：ALTER TABLE 其余子命令（ALTER COLUMN TYPE、IF [NOT] EXISTS、OWNER TO、SET TABLESPACE/LOGGED/STORAGE、CLUSTER ON、REPLICA IDENTITY、自然语法 ADD COLUMN）、CREATE TABLE 其余选项（access method、identity、PARTITION BY 执行）、CREATE TYPE（range/base/shell）、CREATE STATISTICS 的 ndistinct/mcv 算法与 catalog 落地、聚合/窗口函数完整集等。
