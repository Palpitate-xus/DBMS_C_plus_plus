...
| 🔄 4.19 补全数学/字符串/日期时间/JSON/XML/Array/Range 函数全集 | 3.3~3.6 | 字符串函数集已扩充（`tests/string_functions_test.cpp`）：char_length/octet_length/bit_length、substr、lpad/rpad、btrim、split_part、strpos、initcap、to_hex、concat_ws、starts_with、translate、overlay、quote_literal/quote_ident，并为 trim/ltrim/rtrim 增加可选字符集参数；数学函数集已扩充（`tests/math_functions_test.cpp`）：pow/ceiling、log(1/2 参)/log10、trunc(x,n)、degrees/radians/cot、sinh/cosh/tanh/asinh/acosh/atanh、gcd/lcm、div、factorial、width_bucket；日期时间函数集已扩充（`tests/date_functions_test.cpp`）：extract/date_part 全字段、current_* 家族、make_date/time/timestamp、date_trunc；JSON/XML/Array/Range 函数全集待补 |
...
- **Wave 4 函数库 — JSON 函数（4.19f，本次完成）**：
  ...
  - 🔄 仍待后续：`->`/`->>`/`#>` 运算符、`json_extract_path(_text)`、`jsonb_set`/`jsonb_insert`、`json_object_keys`/`json_each`（集合返回）、SQL/JSON `JSON_QUERY`/`JSON_VALUE`/`JSON_TABLE`、JSON 解析校验。
- **Wave 4 仍进行中**：ALTER TABLE 全量子命令、CREATE TABLE 其余选项（access method、identity、PARTITION BY 执行）、CREATE TYPE（range/base/shell）、CREATE STATISTICS 的 ndistinct/mcv 算法与 catalog 落地、Range 函数等。
