...
- 窗口执行已具备可复用的 `WindowOp`/`WindowAgg` 计划节点：无 frame 的 `row_number`、`rank`、`dense_rank`、`lag`、`lead` 已从主 SQL 入口接入 Volcano，覆盖每个窗口独立的分区/排序和最终结果排序；窗口聚合、显式 frame/exclusion、RANGE/GROUPS、复杂目标列表和 OFFSET 仍保留 legacy fallback。
