...
继续将复杂 `INSERT SELECT`、复合/部分/索引推断 conflict target、引用目标表列/子查询的 `ON CONFLICT DO UPDATE` 表达式、`DO UPDATE WHERE`、复杂/子查询/窗口 `RETURNING`、多表 DML、`MERGE` 和复杂表达式迁移到结构化执行器，并在每个边界删除对应 legacy 分支；简单单表 INSERT SELECT、无 target 的 `ON CONFLICT DO NOTHING`、单列主键/唯一列 target 的常量或只引用 `excluded` 的 evaluator 受限标量表达式 `DO UPDATE`、列投影和 evaluator 支持的受限标量表达式 RETURNING 已接入。
