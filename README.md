...
| **锁管理器并发安全** | ✅ PASS | 双线程死锁检测只释放一个受害者；表锁重入/升级、行/页锁 token 归属、跨 backend/跨进程表锁共享、跨 database 隔离和 backend-local 超时均经真实回归 |
...
| [test-report.md](docs/test-report.md) | 自动测试报告（当前回归基线 PASS=137 FAIL=0） |
