...
### 编译
```bash
g++ -std=c++17 -O2 -pthread main.cpp TableManage.cpp Page.cpp PageAllocator.cpp \
    BPTree.cpp BufferPool.cpp LockManager.cpp ExecutionPlan.cpp NetworkServer.cpp \
    -o dbms_main
```
...
├── TableManage.cpp       # 存储引擎实现（DDL/DML/索引/事务/权限/统计）
├── ExecutionPlan.h       # 查询执行计划框架（火山模型算子）
├── ExecutionPlan.cpp     # 查询优化器与 EXPLAIN
